...
from daily_summary
{% if is_incremental() %}
    /* This is an incremental model, so we filter based on the date_of_business 
         to only include new records since the last run. 
         The min_date and max_date variables are set in the Dagster job configuration.
    */
